...
class Prosty {
	...
	***************************/	
	function createNewProject($data){
		...
		$this->recursive_copy($full_path_to_dev, $full_path_to_prod."/1"); 				
		...
		$vhost_nginx_content = $this->vhost_nginx($project_alias, $data["primary_domain"], $data["dev_domain"], $data["additional_domains"]);		
	...
	***************************/	
	...
	// add error to array
	function logError($message, $calling_function){        
    ...
    function checkGitPull($git_response){
        if($git_response[0]>0){
	        $this->logError($git_response[1], __function__);     
        }
    }    		
 	
	// return git remote
	function getGitRemote($project_alias){
	...
	// update screenshot
	function update_screenshot($hostnames, $project_alias){
	...
	***************************/				
	...
	function wp_get_latest($project_alias) {
	 ...
	 */
	function downloadZip($project_alias, $branch){	
    ...
    *********/
	function get_vhost($filename, $fields, $check=true){
		...
		if(count($errors)>0){
	        $this->logError("Following string missing in vhost: ".implode("<br>", $errors), __function__);
        }
	...
	function writeToFile($filename, $content){
		...
		if($res === false){
	        $this->logError("$filename could not be updated", __function__);
		}
	}		
	...
	function vhost_nginx_cache($project_alias, $use_cache){
		if($use_cache == 1){
	...
	// create nginx vhost
	function vhost_nginx($project_alias, $primary_domain, $dev_domain, $additional_domains){
	$use_cache = 0;
	...
	server {
	'.$this->vhost_nginx_additional($primary_domain, $additional_domains).'
	'.$this->vhost_nginx_rewrite($primary_domain).'	
	}
	...
	server {
		...
		# variables
		##############	
	'.$this->vhost_nginx_primary($primary_domain).'
		root				/srv/www/'.$project_alias.'/prod/current;
	'.$this->vhost_nginx_cache($project_alias, $use_cache).'	
	...
	# dev
	server { 
	'.$this->vhost_nginx_dev($dev_domain).'	
